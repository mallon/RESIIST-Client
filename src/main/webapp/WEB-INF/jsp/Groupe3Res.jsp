...
<body>
	...
	<div class="jumbotron">
		<div class="container">
					...
					<table class="w-auto table table-sm" border="1">
						...
						<c:forTokens items="${listdec}" delims="," var="dec"
							...
							<tr>
								...
								<td><c:out value="${dec}"></c:out></td>
								<td><c:out value="${resAHPTop[stat2.index]}"></c:out></td>
								<td><c:out value="${rangAHPTop[stat2.index]}"></c:out>
							</tr>
						</c:forTokens>
					...
					<table class="w-auto table table-sm" border="1">
						<thead class="thead-dark">
							...
							<c:forTokens items="${listdec}" delims="," var="dec"
								...
								<th><c:out value="${dec}" /></th>
							</c:forTokens>

							<c:forTokens items="${listdec}" delims="," var="d"
								varStatus="stat2">
								<tr>
									<th><c:out value="${d}" /></th>
									<c:forEach var="i" begin="0" end="${nbdec-1}" step="1">
										<td><c:out value="${resElectre[stat2.index][i]}"></c:out>
										</td>
									</c:forEach>
					...
					<table class="w-auto table table-sm" border="1">
						...
						<c:forTokens items="${listdec}" delims="," var="dec"
							...
							<tr>
								...
								<td><c:out value="${dec}"></c:out></td>
								<td><c:out value="${resTopsis[stat2.index]}"></c:out></td>
								<td><c:out value="${rangTop[stat2.index]}"></c:out></td>
					...
					<table class="w-auto table table-sm" border="1">
						...
						<c:forTokens items="${listdec}" delims="," var="dec"
							...
							<tr>
								...
								<td><c:out value="${dec}"></c:out></td>
								<td><c:out value="${FluxNega[stat2.index]}"></c:out></td>
								<td><c:out value="${FluxPosit[stat2.index]}"></c:out></td>
								<td><c:out value="${FluxTot[stat2.index]}"></c:out></td>
			...
			<form action="/telechargement3" method="post">
